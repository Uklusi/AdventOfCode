...
class Interpreter(Thread):
    ...
    Syntax: Interpreter(intCodeTape, interactive=False, inputs=[], debug=False)
    ...
    The interpreter has two main modes: interactive and non-interactive.
    
    The interactive mode asks for inputs from stdin and prints outputs to stdout.

    The non-interactive mode (default) is based on a concurrent threading model, with threads sending and receiving inputs from each other.
    ...
    def run(self) -> None:
        while self.step() and not self._stopFlag:
            pass
...
def readTape(file: str = "input.txt") -> list[int]:
    """
...
def stringToAscii(s: str) -> list[int]:
    if s[-1] != "\n":
        s += "\n"
    return [ord(c) for c in s]

def prepareAsciiInput(strings: Union[list[str], str]) -> list[int]:
    if isinstance(strings, str):
        strings = [strings]
    return list(chain.from_iterable( [stringToAscii(s) for s in strings] ))

def asciiToChar(n: int) -> str:
    return chr(n)
