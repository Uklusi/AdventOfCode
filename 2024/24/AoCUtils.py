...
from itertools import product, starmap, chain  # noqa: F401
...
# classes for problem execution and debug
...
class InputReader:
    ...
    def paragraph_words(self, additional_chars="") -> list[list[list[str]]]:
        """
        All words made by contiguous alphanum or additional_chars, separated by paragraphs
        """
        return [
            [re.findall(r"[a-zA-Z0-9" + additional_chars + r"]+", line) for line in par]
            for par in self.paragraphs()
        ]
...
class Vector(_BaseR2Class):
    ...
    def rotate_dir(self, ccw: bool = False):
        assert self.distance() == 1
        return Vector.fromDirection(
            self.directionIndicator() + (1 if not ccw else -1), reverseY=self.reverseY
        )

    @classmethod
    def fromDirection(
        cls, direction: DirectionType, n: int = 1, reverseY: bool = True
    ) -> Vector:
        """
...
class Frame:
    ...
    def where(
        ...
        occupied: Callable[[Position], bool] | None = None,
    ):
        for p in self.get_map_position():
            if predicate(self[p]):
                yield p
    ...
    def where_single(
        ...
        ret = list(self.where(predicate, occupied))
        assert len(ret) == 1
        return ret[0]
...
def aStar(  # type:ignore
    ...
    estimateFunction: Callable[[Position, Position], int] = lambda p, q: p.distance(q),
