...
def solve_p1(useExample: bool = False) -> str:
    ...
    for p in frame.get_map_position():
        if frame[p] == "#" and len(p.adjacent()) > 1:
            for q1, q2 in product(p.adjacent(), repeat=2):
                newdist = dist_s[q1] + dist_e[q2] + 2
                if newdist < to_beat:
                    cheats[to_beat - newdist] += 1

    if useExample:
        result = prettifyDict(cheats)
    else:
        result = sum([cheats[k] for k in cheats if k >= 100])
...
def solve_p2(useExample: bool = False) -> str:
    ...
    for q1, q2 in product(dist_s.keys(), dist_e.keys()):
        if q1.distance(q2) <= 20:
            newdist = dist_s[q1] + dist_e[q2] + q1.distance(q2)
            if newdist < to_beat:
                cheats[to_beat - newdist] += 1

    if useExample:
        result = prettifyDict({k: v for k, v in cheats.items() if k >= 50})
    else:
        result = sum([cheats[k] for k in cheats if k >= 100])
