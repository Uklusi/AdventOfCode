...
def solve_p1(useExample: bool = False) -> str:
    ...
    frequency: dict[str, list[MapPosition]] = defaultdict(list)
    for p in frame.get_map_position():
        if frame[p] != ".":
            frequency[frame[p]].append(p)

    resonance: set[MapPosition] = set()

    for freq in frequency:
        for a1, a2 in combinations(frequency[freq], 2):
            v = a1 - a2
            p = a1 + v
            if p.isInLimits():
                resonance.add(p)
