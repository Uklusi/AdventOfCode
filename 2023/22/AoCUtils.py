...
class Interval():
    ...
    def __and__(self, other: Self):
        if self.intersect(other):
            return Self(max(self.a, other.a), min(self.b, other.b))
        else:
            return None
...
class Cube():
    ...
    def intersect(self, other: Self):
        return self.xs.intersect(other.xs) and self.ys.intersect(other.ys) and self.zs.intersect(other.zs)
    
    def __and__(self, other: Self):
        if self.intersect(other):
            return Self(self.xs & other.xs, self.ys & other.ys, self.zs & other.zs)
        else:
            return None
...
def _sliceToRange(item: slice, minRange: int, maxRange: int, step: int = 1) -> range:
    ...
    """
    (start, stop, oldStep) = cast(tuple[int | None, ...], (item.start, item.stop, item.step))
...
def imageConcat(imageIter: Iterable[Image | NDArray[np.str_]], vertical: bool = False) -> Image:
    """
...
def recreatePath(
    start: _Position,
    goal: _Position,
    pathTrace: dict[_Position, _Position]
) -> list[_Position]:
    """
...
def aStar(
    start: _Position,
    goal: _Position,
    distanceFunction: Callable[[Position, Position], int] = lambda p, q: p.distance(q), #type:ignore
    estimateFunction: Callable[[Position, Position], int] = lambda p, q: p.distance(q), #type:ignore
    includeCorners: bool = False,
...
def aStar(
    ...
    pathTrace: dict[_Position, _Position] = {start: start}
